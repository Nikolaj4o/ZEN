...
#[derive(Clone)]
pub struct FullCircuitOpLv3PolyClassification {
    pub x: Vec<u8>,
    pub l1: Vec<Vec<u8>>,
    pub l2: Vec<Vec<u8>>,
    pub z: Vec<u8>,
    pub rho: Fq,
    pub powers_of_beta: Vec<Fq>,
...
impl ConstraintSynthesizer<Fq> for FullCircuitOpLv3PolyClassification {
    fn generate_constraints(self, cs: ConstraintSystemRef<Fq>) -> Result<(), SynthesisError> {
...
#[derive(Clone)]
pub struct FullCircuitOpLv3Poly {
    pub x: Vec<u8>,
    pub l1: Vec<Vec<u8>>,
    pub l2: Vec<Vec<u8>>,
    pub z: Vec<u8>,
...
impl ConstraintSynthesizer<Fq> for FullCircuitOpLv3Poly {
    fn generate_constraints(self, cs: ConstraintSystemRef<Fq>) -> Result<(), SynthesisError> {
        let beta_vars: Vec<FpVar<Fq>> = self.powers_of_beta.clone().iter().map(|x| FpVar::<Fq>::new_input(ark_relations::ns!(cs, "tmp"), || Ok(*x)).unwrap()).collect();
        ...
        let rho_circ = commit_poly.evaluate(&beta_vars[1])?;
